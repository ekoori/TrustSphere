...
const SphereCard = ({ id, name, unions, participants, description, projects, values, onJoin }) => {
  return (
    <div className="sphere-card" onClick={() => window.location.href = `/sphere`}>
        ...
        <div className="sphere-card-left">
          ...
          <div className="sphere-card-participants">
            <span>🔗 {unions.map((union, index) => (
              <a key={index} href="/union" onClick={(e) => e.stopPropagation()}>{union}{index < unions.length - 1 ? ', ' : ''}</a>
            ))}</span>
          </div>
          <div className="sphere-card-participants">
            <span>👤 {participants.slice(0, 3).map((participant, index) => (
              <a key={index} href="/user" onClick={(e) => e.stopPropagation()}>{participant}{index < 2 ? ', ' : ''}</a>
            ))}, <a href="/sphere" onClick={(e) => e.stopPropagation()}>{participants.length - 3} more...</a></span>
          </div>
        </div>
